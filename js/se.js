$(document).ready(function(){
    ...
    $('.note-add').click(function(){
        ...
        title: "Create a Sticky!",
            text: "<textarea id='stick_note'></textarea>",
            html: true,
            showCancelButton: true,
            closeOnConfirm: false,
        },
        function(){
            var note  = $('#stick_note').val();
            if(note.length <= 5){
                swal({title:"Note Deleted", text:"No value was detected", type:"error"});
                return;
            }
            note = note.replace("'","\\'");
            $.post('/index.php/ajax/add_note','note='+note).done(function(msg){
                console.log(msg);
                var n = JSON.parse(msg);
                if(n.success){
                    swal({title:"Note Added", type:"success"});
                    var html='<div class="row text-right"><i class="fa fa-trash" onclick="trash_note('+n.response.id+')"></i></div><div class="row">'+n.response.note+'</div>';
                    $('#sticky_wid').prepend('<div data-sticky-id="'+n.response.id+'" class="col-md-12 stickies">'+html+'</div>');
                }else{
                    swal({title:"Note Added", type: n.response, type:"error"});
                }
            });
        });
    });
...
var action = {
    load:function(id,callback){
        $.get('/index.php/ajax/get_action/'+id,function(ret){
            info.info = ret;
            callback();
        });
    }
