...
class ajax extends controller{
    ...
    //SOCIAL ACTION
    //------------------
    public function get_action($network,$client,$item_id){
        //CHECK IF AJAX
        $this->is_ajax();
        //CHECK IF AUTHED
        switch($network) {
            case 'FACEBOOK':
                $res = $this->fez->mongo->find(array('tokens'))
                        ->in('records')
                        ->where(array('sfid'=>$client))
                        ->go();
                // Get first value of associative array
                $res = reset($res);
                $res = $res['tokens']['facebook_page_id'];
                break;
        ...
        if($auth){
            ...
            $html=  '<div class="col-md-12">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="row widget">
                            <div id="network-info" class="col-md-12">AUTH EXPIRES</div>';
            if($data['expiration']){
                $html .= ' <div id="action-info" class="col-md-12">'.date('m-d-y',$data['expiration']).'</div>';
            }else{
                $html .= ' <div id="action-info" class="col-md-12">Lifetime</div>';
            }
            $html.='    </div>
                   </div>';
            $html .='
                    <div class="col-md-8" id="action-area">
                        <div class="row widget " id="post-area">
                                <div class="col-md-4">
                                    POST
                                </div>
                                <div id="" class="col-md-6 text-right">
                                    <i class="fa fa-newspaper-o"></i>
                                </div>
                                <div id="counter" class="col-md-2"></div>
                                <textarea class="form-control" id="ta_'.strtolower($network).'"></textarea><br />
                                <div class="row">
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    <button class="btn btn-primary col-md-12" onclick="sego_post('."'".$network."'".","."'".$client."'".","."'".$item_id."'".')">POST</button>
                                </div>
                                </div>
                        </div>
                    </div>
            ...
            $query = $this->fez->mongo->find(array('post_id', 'content', 'created_date', 'post_date', 'status'))
                 ->in('posts')
                 ->where(array('sfid'=>$client,'status'=>'SCHEDULED','item_id'=>$item_id))
                 ->go();

            foreach($query as $k => $v){

                $html .= '<div class="row queued-post">';
                $html .= '   <div class="col-md-2">'.date('m-d-y',$v['post_date']).'</div>';
                $html .= '   <div class="col-md-8">'.$v['content'].'</div>';
                $html .= '</div>';
            }

        }else{
            $html = '<div class="col-md-12 text-center">
                        Client is not Authorized to post to '.$network.'<br />
                        <a display="_blank" href="/index.php/sego/'.strtolower($network).'_loader/'.$client.'">Auth Client</a>
                    </div>
            ';
        }
